...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        100,
        "环境变化主动触发技能并推送结果。",
        "LaunchAgent、消息/卡片事件、多维表轮询、Webhook HMAC 入队；声明式定时触发器（daily/weekly/cron/once）由 feishu serve 轮询并启动 Agent v2 后台任务；CLI `feishu triggers` 管理。",
        "待补：条件触发器（消息模式/多维表变更）、触发器运行日志可视化。",
