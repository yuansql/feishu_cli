...
CAPABILITIES: tuple[Capability, ...] = (
    Capability(
        ...
        "TaskRunner v2 已支持 observe→plan→act→verify→replan、确认闸、取消、SQLite 索引与租约恢复。",
        "待补：规划质量依赖本地模型；长任务租约默认 120s。",
    ...
    Capability(
        ...
        "本地 researcher/executor/writer：按目标选角色子集，确定性路径并行生成 brief。",
        "待补：复杂任务并行子规划与领域专用角色扩展。",
        "https://www.feishu.cn/content/article/7585126677299137754",
    ),
    Capability(
        "feishu_tools",
    ...
    Capability(
        ...
        "JSONL traces + feishu eval + feishu runs（SQLite 运行/租约一览）。",
        "待补：token 汇总（无 Web 台）。",
    ...
    Capability(
        ...
        "user/bot 隔离、doctor scope 探针、确认闸、脱敏日志、本机身份闸（feishu setup）。",
    ...
    Capability(
        ...
        "Agents.md + experience.jsonl；规划前注入；任务完成自动记经验；feishu memory CLI。",
        "待补：按群隔离经验、自动从 Badcase 提炼 Agents.md 段落。",
        "https://www.feishu.cn/content/article/7585126677299137754",
    ),
)
