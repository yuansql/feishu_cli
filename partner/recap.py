...
from .formatters import _items, format_lark_error
from .ids import P2P_CHAT_ID, USER_OPEN_ID
...
def collect_today_evidence(now: datetime | None = None) -> EvidenceBundle:
    ...
    for _score, chat_name, rows in sorted(
        ...
        chunk: list[str] = []
        limit = 14 if "飞书 CLI" in chat_name else 8
        ...
        for message in sorted(ranked, key=_when):
            ...
            chunk.append(
                ...
                f"[{_when(message):%H:%M}][{role}] {body[:220]}"
            )
            if len(chunk) >= limit or evidence_count >= 90:
                break
        if not chunk:
            continue
        lines.append(f"\n## {chat_name}")
        lines.extend(chunk)
        if evidence_count >= 90:
            break
